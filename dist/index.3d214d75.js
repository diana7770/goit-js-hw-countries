...
function createCountry(countries) {
    if (countries.length === 1) countries.forEach((country)=>{
    ...
    else if (countries.length > 1) countries.forEach((country)=>{
        const li = document.createElement("li");
...
function handleInput() {
    ...
    fetchCountries(urlName, inputValue).then((data)=>{
        if (data.length > 10) {
            PNotify.error({});
            return;
        }
...
fetchCountries(urlName).then((data)=>createCountry(data)).catch((error)=>console.error("Error fetching countries:", error));
...
},{}]},["km5uZ","bB7Pu"], "bB7Pu", "parcelRequire4b08")
